...
export async function getBalanceSumInsured(
  ...
  try {
    ...
    // Step 3 — MemberUtilization → Utilized / Blocked / Reserved per BPSIID
    // PropertyValueID: 2 = Utilized, 3 = Reserved, 4 = Blocked
    ...
    const utilRows = await queryRows(
      ...
      `SELECT
         ...
         ISNULL(mu.PropertyValueID, 2)                AS Type,
    ...
    const rows: BSISumInsuredRow[] = memberSiRows.map((si) => {
      ...
      for (const u of myUtils) {
        ...
        const utlAmt  = Number(u.Utilized) || 0;
        const blkAmt  = Number(u.Blocked)  || 0;

        if (type === 2) utilized += utlAmt;
        else if (type === 3) reserved += utlAmt;
        else if (type === 4) blocked  += blkAmt || utlAmt;
        ...
        utilization.push({
          ...
          Type:           type,
          Amount:         utlAmt || blkAmt,
          StageID:        Number(u.StageID)         || 0,
          CRDR:           true,
        });
      }
